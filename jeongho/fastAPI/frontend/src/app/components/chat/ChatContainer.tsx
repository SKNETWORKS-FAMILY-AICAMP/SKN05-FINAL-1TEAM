...
export function ChatContainer() {
  ...
  const [currentNodes, setCurrentNodes] = useState<{ message_id: number; currentNodes: string[]; }>({
    message_id: 1,
    currentNodes: ["임시 노드 A", "임시 노드 B", "임시 노드 C"],
  });
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            else if (data.currentNode === "END") {
              setIsReverseQ(false)
              accumulatedAnswer += data.answer;
              setCurrentAnswer(accumulatedAnswer);
              
              setCurrentNodes({
                message_id: userMessageId, // ✅ 새로운 message_id 값 설정
                currentNodes: ["END"], // ✅ currentNodes를 "END"만 포함하도록 초기화
              });
