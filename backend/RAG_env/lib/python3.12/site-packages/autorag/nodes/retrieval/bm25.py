...
class BM25(BaseRetrieval):
	...
	def _pure(
		...
		if ids is not None:
			# 기존 로직 (이미 주어진 passage ID에 대해 점수 계산)
			...
		
		# 비동기 BM25 검색 수행 시 selected_model 인자도 함께 전달
		tasks = [
			bm25_pure(
				input_queries,
				top_k,
...
async def bm25_pure(
    queries: List[str], top_k: int, tokenizer, bm25_api: BM25Okapi, bm25_corpus: Dict, selected_model: str
) -> Tuple[List[str], List[float]]:
    tokenized_queries = tokenize(queries, tokenizer)
    id_result = []
    score_result = []
    for query in tokenized_queries:
        ...
        for idx in sorted_indices:
            # 여기서 passage_id와 메타데이터(예: "model")를 확인
            passage_id = bm25_corpus["passage_id"][idx]
            passage_model = bm25_corpus.get("model", [None] * len(bm25_corpus["passage_id"]))[idx]
            if selected_model is None or passage_model == selected_model:
                filtered_ids.append(passage_id)
                filtered_scores.append(scores[idx])
            if len(filtered_ids) >= top_k:
                break
        # 만약 필터링 후 top_k 미만이면 그대로 사용하거나, 후처리로 채울 수 있음.
        id_result.append(filtered_ids)
        score_result.append(filtered_scores)
    # 나머지 후처리(예: evenly_distribute_passages) 적용
    id_result, score_result = evenly_distribute_passages(id_result, score_result, top_k)
    # 최종 정렬 후 반환
    result = [
        (_id, score)
		...
		new_bm25_corpus = pd.DataFrame(
			{
				...
				"model": new_passage["metadata"].apply(lambda x: x.get('model') if isinstance(x, dict) else None).tolist(),
